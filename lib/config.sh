...
apply_env_overrides() {
	...
	[[ -n "${EMDASH_INSTALL_BACKUP_SFTP_HOST:-}" ]] && BACKUP_SFTP_HOST="${EMDASH_INSTALL_BACKUP_SFTP_HOST}"
	[[ -n "${EMDASH_INSTALL_BACKUP_SFTP_PORT:-}" ]] && BACKUP_SFTP_PORT="${EMDASH_INSTALL_BACKUP_SFTP_PORT}"
	[[ -n "${EMDASH_INSTALL_BACKUP_SFTP_USER:-}" ]] && BACKUP_SFTP_USER="${EMDASH_INSTALL_BACKUP_SFTP_USER}"
	[[ -n "${EMDASH_INSTALL_BACKUP_SFTP_PASSWORD:-}" ]] && BACKUP_SFTP_PASSWORD="${EMDASH_INSTALL_BACKUP_SFTP_PASSWORD}"
	[[ -n "${EMDASH_INSTALL_BACKUP_SFTP_REMOTE_PATH:-}" ]] && BACKUP_SFTP_REMOTE_PATH="${EMDASH_INSTALL_BACKUP_SFTP_REMOTE_PATH}"
...
validate_config() {
	...
	*) fail "不支持的备份目标: ${BACKUP_TARGET_TYPE}" ;;
	...
	if [[ "${BACKUP_ENABLED}" == "1" && "${BACKUP_TARGET_TYPE}" == "sftp" ]]; then
		[[ -n "${BACKUP_SFTP_HOST}" ]] || fail "SFTP 备份必须提供主机。"
		[[ -n "${BACKUP_SFTP_USER}" ]] || fail "SFTP 备份必须提供用户。"
		[[ -n "${BACKUP_SFTP_REMOTE_PATH}" ]] || fail "SFTP 备份必须提供远端目录。"
	fi
