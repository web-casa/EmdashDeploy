...
ti() {
	...
	summary_generated_only) case "${lang}" in en) printf 'Configuration generated.' ;; ja) printf '設定を生成しました。' ;; ko) printf '구성이 생성되었습니다.' ;; es) printf 'Configuración generada.' ;; de) printf 'Konfiguration erstellt.' ;; fr) printf 'Configuration générée.' ;; zh-CN) printf '已生成配置。' ;; zh-TW) printf '已產生設定。' ;; pt) printf 'Configuração gerada.' ;; esac ;;
