...
ti() {
	...
	unsupported_template) case "${lang}" in en) printf 'Unsupported template: %s' ;; ja) printf 'サポートされていないテンプレート: %s' ;; ko) printf '지원되지 않는 템플릿: %s' ;; es) printf 'Plantilla no soportada: %s' ;; de) printf 'Nicht unterstütztes Template: %s' ;; fr) printf 'Modèle non pris en charge : %s' ;; zh-CN) printf '不支持的模板: %s' ;; zh-TW) printf '不支援的範本: %s' ;; pt) printf 'Template não suportado: %s' ;; esac ;;
	unsupported_db) case "${lang}" in en) printf 'Unsupported database driver: %s' ;; ja) printf 'サポートされていないデータベースドライバー: %s' ;; ko) printf '지원되지 않는 데이터베이스 드라이버: %s' ;; es) printf 'Controlador de base de datos no soportado: %s' ;; de) printf 'Nicht unterstützter Datenbanktreiber: %s' ;; fr) printf 'Pilote de base de données non pris en charge : %s' ;; zh-CN) printf '不支持的数据库驱动: %s' ;; zh-TW) printf '不支援的資料庫驅動: %s' ;; pt) printf 'Driver de banco de dados não suportado: %s' ;; esac ;;
	unsupported_session) case "${lang}" in en) printf 'Unsupported session driver: %s' ;; ja) printf 'サポートされていないセッションドライバー: %s' ;; ko) printf '지원되지 않는 세션 드라이버: %s' ;; es) printf 'Controlador de sesión no soportado: %s' ;; de) printf 'Nicht unterstützter Sitzungstreiber: %s' ;; fr) printf 'Pilote de session non pris en charge : %s' ;; zh-CN) printf '不支持的会话驱动: %s' ;; zh-TW) printf '不支援的工作階段驅動: %s' ;; pt) printf 'Driver de sessão não suportado: %s' ;; esac ;;
	unsupported_storage) case "${lang}" in en) printf 'Unsupported storage driver: %s' ;; ja) printf 'サポートされていないストレージドライバー: %s' ;; ko) printf '지원되지 않는 스토리지 드라이버: %s' ;; es) printf 'Controlador de almacenamiento no soportado: %s' ;; de) printf 'Nicht unterstützter Speichertreiber: %s' ;; fr) printf 'Pilote de stockage non pris en charge : %s' ;; zh-CN) printf '不支持的存储驱动: %s' ;; zh-TW) printf '不支援的儲存驅動: %s' ;; pt) printf 'Driver de armazenamento não suportado: %s' ;; esac ;;
	unsupported_backup_target) case "${lang}" in en) printf 'Unsupported backup target: %s' ;; ja) printf 'サポートされていないバックアップ先: %s' ;; ko) printf '지원되지 않는 백업 대상: %s' ;; es) printf 'Destino de respaldo no soportado: %s' ;; de) printf 'Nicht unterstütztes Sicherungsziel: %s' ;; fr) printf 'Cible de sauvegarde non prise en charge : %s' ;; zh-CN) printf '不支持的备份目标: %s' ;; zh-TW) printf '不支援的備份目標: %s' ;; pt) printf 'Destino de backup não suportado: %s' ;; esac ;;
	pg_username_invalid) case "${lang}" in en) printf 'PostgreSQL username must start with a letter or underscore, and contain only letters, digits, and underscores.' ;; ja) printf 'PostgreSQLのユーザー名は英字またはアンダースコアで始まり、英字・数字・アンダースコアのみを含む必要があります。' ;; ko) printf 'PostgreSQL 사용자 이름은 문자 또는 밑줄로 시작해야 하며, 문자, 숫자, 밑줄만 포함할 수 있습니다.' ;; es) printf 'El nombre de usuario de PostgreSQL debe comenzar con una letra o guion bajo, y contener solo letras, dígitos y guiones bajos.' ;; de) printf 'Der PostgreSQL-Benutzername muss mit einem Buchstaben oder Unterstrich beginnen und darf nur Buchstaben, Ziffern und Unterstriche enthalten.' ;; fr) printf "Le nom d'utilisateur PostgreSQL doit commencer par une lettre ou un tiret bas, et ne contenir que des lettres, des chiffres et des tirets bas." ;; zh-CN) printf 'PostgreSQL 用户名必须以字母或下划线开头，且只能包含字母、数字和下划线。' ;; zh-TW) printf 'PostgreSQL 使用者名稱必須以字母或底線開頭，且只能包含字母、數字和底線。' ;; pt) printf 'O nome de usuário do PostgreSQL deve começar com uma letra ou sublinhado, e conter apenas letras, dígitos e sublinhados.' ;; esac ;;
	pg_dbname_invalid) case "${lang}" in en) printf 'PostgreSQL database name must start with a letter or underscore, and contain only letters, digits, and underscores.' ;; ja) printf 'PostgreSQLのデータベース名は英字またはアンダースコアで始まり、英字・数字・アンダースコアのみを含む必要があります。' ;; ko) printf 'PostgreSQL 데이터베이스 이름은 문자 또는 밑줄로 시작해야 하며, 문자, 숫자, 밑줄만 포함할 수 있습니다.' ;; es) printf 'El nombre de la base de datos PostgreSQL debe comenzar con una letra o guion bajo, y contener solo letras, dígitos y guiones bajos.' ;; de) printf 'Der PostgreSQL-Datenbankname muss mit einem Buchstaben oder Unterstrich beginnen und darf nur Buchstaben, Ziffern und Unterstriche enthalten.' ;; fr) printf 'Le nom de la base de données PostgreSQL doit commencer par une lettre ou un tiret bas, et ne contenir que des lettres, des chiffres et des tirets bas.' ;; zh-CN) printf 'PostgreSQL 数据库名称必须以字母或下划线开头，且只能包含字母、数字和下划线。' ;; zh-TW) printf 'PostgreSQL 資料庫名稱必須以字母或底線開頭，且只能包含字母、數字和底線。' ;; pt) printf 'O nome do banco de dados PostgreSQL deve começar com uma letra ou sublinhado, e conter apenas letras, dígitos e sublinhados.' ;; esac ;;
	caddy_requires_domain) case "${lang}" in en) printf 'DOMAIN is required when Caddy is enabled.' ;; ja) printf 'Caddyが有効な場合、DOMAINは必須です。' ;; ko) printf 'Caddy가 활성화된 경우 DOMAIN이 필요합니다.' ;; es) printf 'DOMAIN es obligatorio cuando Caddy está habilitado.' ;; de) printf 'DOMAIN ist erforderlich, wenn Caddy aktiviert ist.' ;; fr) printf 'DOMAIN est requis lorsque Caddy est activé.' ;; zh-CN) printf '启用 Caddy 时必须设置 DOMAIN。' ;; zh-TW) printf '啟用 Caddy 時必須設定 DOMAIN。' ;; pt) printf 'DOMAIN é obrigatório quando o Caddy está habilitado.' ;; esac ;;
	https_requires_email) case "${lang}" in en) printf 'ADMIN_EMAIL is required when HTTPS is enabled.' ;; ja) printf 'HTTPSが有効な場合、ADMIN_EMAILは必須です。' ;; ko) printf 'HTTPS가 활성화된 경우 ADMIN_EMAIL이 필요합니다.' ;; es) printf 'ADMIN_EMAIL es obligatorio cuando HTTPS está habilitado.' ;; de) printf 'ADMIN_EMAIL ist erforderlich, wenn HTTPS aktiviert ist.' ;; fr) printf 'ADMIN_EMAIL est requis lorsque HTTPS est activé.' ;; zh-CN) printf '启用 HTTPS 时必须设置 ADMIN_EMAIL。' ;; zh-TW) printf '啟用 HTTPS 時必須設定 ADMIN_EMAIL。' ;; pt) printf 'ADMIN_EMAIL é obrigatório quando o HTTPS está habilitado.' ;; esac ;;
	s3_requires_endpoint) case "${lang}" in en) printf 'S3 storage requires an endpoint.' ;; ja) printf 'S3ストレージにはエンドポイントが必要です。' ;; ko) printf 'S3 스토리지에는 엔드포인트가 필요합니다.' ;; es) printf 'El almacenamiento S3 requiere un endpoint.' ;; de) printf 'S3-Speicher erfordert einen Endpunkt.' ;; fr) printf 'Le stockage S3 nécessite un point de terminaison.' ;; zh-CN) printf 'S3 存储需要设置端点。' ;; zh-TW) printf 'S3 儲存需要設定端點。' ;; pt) printf 'O armazenamento S3 requer um endpoint.' ;; esac ;;
	s3_requires_bucket) case "${lang}" in en) printf 'S3 storage requires a bucket.' ;; ja) printf 'S3ストレージにはバケットが必要です。' ;; ko) printf 'S3 스토리지에는 버킷이 필요합니다.' ;; es) printf 'El almacenamiento S3 requiere un bucket.' ;; de) printf 'S3-Speicher erfordert einen Bucket.' ;; fr) printf 'Le stockage S3 nécessite un bucket.' ;; zh-CN) printf 'S3 存储需要设置存储桶。' ;; zh-TW) printf 'S3 儲存需要設定儲存桶。' ;; pt) printf 'O armazenamento S3 requer um bucket.' ;; esac ;;
	s3_requires_access_key) case "${lang}" in en) printf 'S3 storage requires an access key.' ;; ja) printf 'S3ストレージにはアクセスキーが必要です。' ;; ko) printf 'S3 스토리지에는 액세스 키가 필요합니다.' ;; es) printf 'El almacenamiento S3 requiere una clave de acceso.' ;; de) printf 'S3-Speicher erfordert einen Zugriffsschlüssel.' ;; fr) printf "Le stockage S3 nécessite une clé d'accès." ;; zh-CN) printf 'S3 存储需要设置访问密钥。' ;; zh-TW) printf 'S3 儲存需要設定存取金鑰。' ;; pt) printf 'O armazenamento S3 requer uma chave de acesso.' ;; esac ;;
	s3_requires_secret_key) case "${lang}" in en) printf 'S3 storage requires a secret key.' ;; ja) printf 'S3ストレージにはシークレットキーが必要です。' ;; ko) printf 'S3 스토리지에는 시크릿 키가 필요합니다.' ;; es) printf 'El almacenamiento S3 requiere una clave secreta.' ;; de) printf 'S3-Speicher erfordert einen geheimen Schlüssel.' ;; fr) printf 'Le stockage S3 nécessite une clé secrète.' ;; zh-CN) printf 'S3 存储需要设置秘密密钥。' ;; zh-TW) printf 'S3 儲存需要設定密碼金鑰。' ;; pt) printf 'O armazenamento S3 requer uma chave secreta.' ;; esac ;;
	s3_backup_requires_endpoint) case "${lang}" in en) printf 'S3 backup requires an endpoint.' ;; ja) printf 'S3バックアップにはエンドポイントが必要です。' ;; ko) printf 'S3 백업에는 엔드포인트가 필요합니다.' ;; es) printf 'El respaldo S3 requiere un endpoint.' ;; de) printf 'S3-Sicherung erfordert einen Endpunkt.' ;; fr) printf 'La sauvegarde S3 nécessite un point de terminaison.' ;; zh-CN) printf 'S3 备份需要设置端点。' ;; zh-TW) printf 'S3 備份需要設定端點。' ;; pt) printf 'O backup S3 requer um endpoint.' ;; esac ;;
	s3_backup_requires_bucket) case "${lang}" in en) printf 'S3 backup requires a bucket.' ;; ja) printf 'S3バックアップにはバケットが必要です。' ;; ko) printf 'S3 백업에는 버킷이 필요합니다.' ;; es) printf 'El respaldo S3 requiere un bucket.' ;; de) printf 'S3-Sicherung erfordert einen Bucket.' ;; fr) printf 'La sauvegarde S3 nécessite un bucket.' ;; zh-CN) printf 'S3 备份需要设置存储桶。' ;; zh-TW) printf 'S3 備份需要設定儲存桶。' ;; pt) printf 'O backup S3 requer um bucket.' ;; esac ;;
	s3_backup_requires_access_key) case "${lang}" in en) printf 'S3 backup requires an access key.' ;; ja) printf 'S3バックアップにはアクセスキーが必要です。' ;; ko) printf 'S3 백업에는 액세스 키가 필요합니다.' ;; es) printf 'El respaldo S3 requiere una clave de acceso.' ;; de) printf 'S3-Sicherung erfordert einen Zugriffsschlüssel.' ;; fr) printf "La sauvegarde S3 nécessite une clé d'accès." ;; zh-CN) printf 'S3 备份需要设置访问密钥。' ;; zh-TW) printf 'S3 備份需要設定存取金鑰。' ;; pt) printf 'O backup S3 requer uma chave de acesso.' ;; esac ;;
	s3_backup_requires_secret_key) case "${lang}" in en) printf 'S3 backup requires a secret key.' ;; ja) printf 'S3バックアップにはシークレットキーが必要です。' ;; ko) printf 'S3 백업에는 시크릿 키가 필요합니다.' ;; es) printf 'El respaldo S3 requiere una clave secreta.' ;; de) printf 'S3-Sicherung erfordert einen geheimen Schlüssel.' ;; fr) printf 'La sauvegarde S3 nécessite une clé secrète.' ;; zh-CN) printf 'S3 备份需要设置秘密密钥。' ;; zh-TW) printf 'S3 備份需要設定密碼金鑰。' ;; pt) printf 'O backup S3 requer uma chave secreta.' ;; esac ;;
	cron_no_newline) case "${lang}" in en) printf 'Backup cron must not contain newlines.' ;; ja) printf 'バックアップcronに改行を含めることはできません。' ;; ko) printf '백업 cron에 줄바꿈을 포함할 수 없습니다.' ;; es) printf 'El cron de respaldo no debe contener saltos de línea.' ;; de) printf 'Der Backup-Cron darf keine Zeilenumbrüche enthalten.' ;; fr) printf 'Le cron de sauvegarde ne doit pas contenir de sauts de ligne.' ;; zh-CN) printf '备份 cron 不能包含换行符。' ;; zh-TW) printf '備份 cron 不能包含換行符號。' ;; pt) printf 'O cron de backup não pode conter quebras de linha.' ;; esac ;;
	cron_five_fields) case "${lang}" in en) printf 'Backup cron must have exactly 5 fields.' ;; ja) printf 'バックアップcronはちょうど5つのフィールドが必要です。' ;; ko) printf '백업 cron은 정확히 5개의 필드가 필요합니다.' ;; es) printf 'El cron de respaldo debe tener exactamente 5 campos.' ;; de) printf 'Der Backup-Cron muss genau 5 Felder haben.' ;; fr) printf 'Le cron de sauvegarde doit comporter exactement 5 champs.' ;; zh-CN) printf '备份 cron 必须恰好包含 5 个字段。' ;; zh-TW) printf '備份 cron 必須恰好包含 5 個欄位。' ;; pt) printf 'O cron de backup deve ter exatamente 5 campos.' ;; esac ;;
	cron_invalid_field) case "${lang}" in en) printf 'Backup cron contains an unsupported field: %s' ;; ja) printf 'バックアップcronにサポートされていないフィールドがあります: %s' ;; ko) printf '백업 cron에 지원되지 않는 필드가 포함되어 있습니다: %s' ;; es) printf 'El cron de respaldo contiene un campo no soportado: %s' ;; de) printf 'Der Backup-Cron enthält ein nicht unterstütztes Feld: %s' ;; fr) printf 'Le cron de sauvegarde contient un champ non pris en charge : %s' ;; zh-CN) printf '备份 cron 包含不支持的字段: %s' ;; zh-TW) printf '备份 cron 包含不支援的欄位: %s' ;; pt) printf 'O cron de backup contém um campo não suportado: %s' ;; esac ;;
	template_dir_missing) case "${lang}" in en) printf 'Template directory does not exist: %s' ;; ja) printf 'テンプレートディレクトリが存在しません: %s' ;; ko) printf '템플릿 디렉터리가 존재하지 않습니다: %s' ;; es) printf 'El directorio de plantillas no existe: %s' ;; de) printf 'Template-Verzeichnis existiert nicht: %s' ;; fr) printf "Le répertoire de modèles n'existe pas : %s" ;; zh-CN) printf '模板目录不存在: %s' ;; zh-TW) printf '範本目錄不存在: %s' ;; pt) printf 'O diretório de templates não existe: %s' ;; esac ;;
	existing_site_missing_metadata) case "${lang}" in en) printf 'Existing site detected but install.yml metadata is missing. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトが検出されましたが、install.ymlメタデータがありません。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트가 감지되었지만 install.yml 메타데이터가 없습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'Se detectó un sitio existente pero faltan los metadatos de install.yml. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website erkannt, aber install.yml-Metadaten fehlen. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf 'Site existant détecté mais les métadonnées install.yml sont manquantes. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer.' ;; zh-CN) printf '检测到现有站点，但缺少 install.yml 元数据。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '偵測到現有站點，但缺少 install.yml 中繼資料。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'Site existente detectado, mas os metadados do install.yml estão ausentes. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	existing_site_template_mismatch) case "${lang}" in en) printf 'Existing site uses template %s, but installer is configured for %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトはテンプレート%sを使用していますが、インストーラーは%s用に設定されています。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트는 템플릿 %s을(를) 사용하지만, 설치 프로그램은 %s(으)로 구성되어 있습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'El sitio existente usa la plantilla %s, pero el instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website verwendet Template %s, aber der Installer ist für %s konfiguriert. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf "Le site existant utilise le modèle %s, mais l'installateur est configuré pour %s. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer." ;; zh-CN) printf '现有站点使用模板 %s，但安装程序配置为 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '現有站點使用範本 %s，但安裝程式設定為 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'O site existente usa o template %s, mas o instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	existing_site_db_mismatch) case "${lang}" in en) printf 'Existing site uses database driver %s, but installer is configured for %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトはデータベースドライバー%sを使用していますが、インストーラーは%s用に設定されています。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트는 데이터베이스 드라이버 %s을(를) 사용하지만, 설치 프로그램은 %s(으)로 구성되어 있습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'El sitio existente usa el controlador de base de datos %s, pero el instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website verwendet Datenbanktreiber %s, aber der Installer ist für %s konfiguriert. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf "Le site existant utilise le pilote de base de données %s, mais l'installateur est configuré pour %s. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer." ;; zh-CN) printf '现有站点使用数据库驱动 %s，但安装程序配置为 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '現有站點使用資料庫驅動 %s，但安裝程式設定為 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'O site existente usa o driver de banco de dados %s, mas o instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	existing_site_session_mismatch) case "${lang}" in en) printf 'Existing site uses session driver %s, but installer is configured for %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトはセッションドライバー%sを使用していますが、インストーラーは%s用に設定されています。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트는 세션 드라이버 %s을(를) 사용하지만, 설치 프로그램은 %s(으)로 구성되어 있습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'El sitio existente usa el controlador de sesión %s, pero el instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website verwendet Sitzungstreiber %s, aber der Installer ist für %s konfiguriert. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf "Le site existant utilise le pilote de session %s, mais l'installateur est configuré pour %s. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer." ;; zh-CN) printf '现有站点使用会话驱动 %s，但安装程序配置为 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '現有站點使用工作階段驅動 %s，但安裝程式設定為 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'O site existente usa o driver de sessão %s, mas o instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	existing_site_storage_mismatch) case "${lang}" in en) printf 'Existing site uses storage driver %s, but installer is configured for %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトはストレージドライバー%sを使用していますが、インストーラーは%s用に設定されています。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트는 스토리지 드라이버 %s을(를) 사용하지만, 설치 프로그램은 %s(으)로 구성되어 있습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'El sitio existente usa el controlador de almacenamiento %s, pero el instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website verwendet Speichertreiber %s, aber der Installer ist für %s konfiguriert. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf "Le site existant utilise le pilote de stockage %s, mais l'installateur est configuré pour %s. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer." ;; zh-CN) printf '现有站点使用存储驱动 %s，但安装程序配置为 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '現有站點使用儲存驅動 %s，但安裝程式設定為 %s。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'O site existente usa o driver de armazenamento %s, mas o instalador está configurado para %s. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	existing_site_missing_astro_config) case "${lang}" in en) printf 'Existing site detected but astro.config.mjs is missing. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 to force.' ;; ja) printf '既存サイトが検出されましたが、astro.config.mjsがありません。強制するにはEMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1を使用してください。' ;; ko) printf '기존 사이트가 감지되었지만 astro.config.mjs가 없습니다. 강제하려면 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1을 사용하세요.' ;; es) printf 'Se detectó un sitio existente pero falta astro.config.mjs. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forzar.' ;; de) printf 'Bestehende Website erkannt, aber astro.config.mjs fehlt. Verwenden Sie EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 zum Erzwingen.' ;; fr) printf 'Site existant détecté mais astro.config.mjs est manquant. Utilisez EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 pour forcer.' ;; zh-CN) printf '检测到现有站点，但缺少 astro.config.mjs。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 强制执行。' ;; zh-TW) printf '偵測到現有站點，但缺少 astro.config.mjs。使用 EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 強制執行。' ;; pt) printf 'Site existente detectado, mas o astro.config.mjs está ausente. Use EMDASH_INSTALL_FORCE_SYNC_TEMPLATE=1 para forçar.' ;; esac ;;
	*)
		printf '%s' "${key}"
		;;
	esac
